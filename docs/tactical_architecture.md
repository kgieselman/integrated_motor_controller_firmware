...
| Field | Value |
|---|---|
| Status | Design. FreeRTOS baseline integrated; application layers not yet written. |
| Target | `imc_tactical` (the bare-metal `imc_bringup` target is unaffected by everything here) |
| Kernel | FreeRTOS V11.1.0, `GCC_ARM_CM33_NTZ_NONSECURE`, 1 kHz tick |
| Control rate | 200 Hz (`dt = 0.005 f`) |
| Language | C++20, `-fno-exceptions -fno-rtti`, no heap allocation after boot (§2.2 — a change from the repo's current C++17) |
...
`integrated_motor_controller_firmware_arch.md` describes layers 1–2 (HAL and drivers), which are already
built and shared with bringup. This document specifies layers 3–7, which are not.
...
|---|---|---|---|---|
| 7 | Robot config | `app/tactical/src/config/` | One file per machine per year. Declares which subsystems exist, constructs them with their drivers, lists their gains. | New |
| 6 | Behavior | `app/tactical/src/behavior/` | Teleop stick mapping; autonomous routines as ordered step lists. | New |
| 5 | Subsystem | `app/tactical/src/subsystems/` | One class per mechanism, all implementing the same five methods. Each owns its actuators exclusively. | New |
| 4 | Control | `app/tactical/src/control/` | PID, slew limiting, expo, filters, velocity and heading estimation. Pure math, no HAL types. | New |
| 3 | Platform | `app/tactical/src/platform/` | Task set, `Snapshot<T>`, `SafetyMonitor`, `InputSource`, `ParamStore`, `Telemetry`. | New |
...
### 2.2 Language standard — move to C++20

The repo currently pins C++17 (`CMakeLists.txt` line 8, `style_guide.md` §1). This design asks for
**C++20**, adopted deliberately for two features and nothing else. `Dockerfile` is `ubuntu:24.04`, which
ships arm-none-eabi-gcc 13.2 — complete C++20 core language support and effectively complete library
support. The change is two lines of CMake plus a style-guide edit, and phase 0 is the moment to make it,
while the only C++ in the repo is the driver layer.
...
The repo adopts **co-located headers and sources** everywhere hand-written code lives — `drivers/` and
`app/` — replacing the current `include/` + `src/` split. `cubemx/` is CubeMX-owned and is not touched.
...
```
drivers/                         # was drivers/include + drivers/src
├── AdcDma.hpp / .cpp
├── Battery.hpp / .cpp
├── CRSFReceiver.hpp / .cpp
├── Drivetrain.hpp / .cpp        # bringup-only after §10.2
├── EEPROM.hpp / .cpp
├── Encoder.hpp / .cpp           # rewrite for input capture — §10.1
├── IMU.hpp / .cpp
├── Motor.hpp / .cpp
...
└── src/
    ├── main_tactical.cpp        # HAL + peripheral init → build robot → create tasks → scheduler
    ├── tasks/
    │   ├── ControlTask.cpp      # the 200 Hz cycle
    │   ├── CommsTask.cpp        # CRSF frames → DriverInput snapshot
    │   ├── TelemetryTask.cpp    # USB-CDC out, parameter console in
    ...
    │   ├── SafetyMonitor.hpp    # the failsafe table, evaluated every cycle
    │   ├── InputSource.hpp      # CRSF channels → deadband, expo, switch decode
    │   ├── ParamStore.hpp       # named tunables, EEPROM persist, CRC + version
    │   └── Telemetry.hpp        # sink, SPSC event ring, text formatter
    ...
    │   ├── LowPassFilter.hpp
    │   ├── Debouncer.hpp
    │   ├── VelocityEstimator.hpp   # counts + dt → RPM, signed from commanded direction
    │   └── HeadingEstimator.hpp    # gyro + accel complementary filter → yaw
    ├── subsystems/
    │   ├── Subsystem.hpp        # the contract
    │   ├── SubsystemManager.hpp # walks the array: periodic / disableAll / telemetry
    │   ├── DriveBase.hpp        # phase 1 open loop, phase 3 velocity PID
    │   ├── Intake.hpp           # phase 5
    │   └── Launcher.hpp         # phase 5
    ├── behavior/
    │   ├── Behavior.hpp         # update(ctx) / isFinished()
    │   ├── TeleopBehavior.hpp   # the only place stick → intent mapping lives
    │   ├── RoutineRunner.hpp    # runs a static array of steps in order
    │   └── steps/               # DriveDistance, TurnToAngle, RunLauncher, Wait
    └── config/
        ├── RobotConfig.hpp      # selects the active year from -DIMC_ROBOT
        ├── Robot2026.hpp
        └── Robot2025.hpp        # kept building as a contract regression check
...
### 10.1 Open issues that gate a phase

Tracked in `integrated_motor_controller_firmware_arch.md` §9; listed here against the phase that trips
over them.

| Phase | Issue |
|---|---|
| 0 | **N1 / F18** — CRSF DMA re-arm collides with the circular DMA config. The comms task is the first thing built and the first thing to hit it. Move UART4 RX DMA to normal mode, or track a tail index into the circular buffer. |
| 0 | **Tactical peripheral init is missing.** `main_tactical.cpp` initialises GPIO only. DMA, UART4, SPI1, I2C1, ADC1 and the timers all need wiring before the scheduler starts — including `HAL_ADCEx_Calibration_Start()` and `HAL_ADC_Start_DMA()` to start `g_adcBuf`. |
| 0 | **Six of the nine drivers are not in any build.** `AdcDma`, `Battery`, `CRSFReceiver`, `Drivetrain`, `Encoder` and `Motor` are never compiled by any target — see §8.3. Fix this first; it is why the stale headers went unnoticed, and phase 0 cannot wire CRSF or the ADC without them. *(New — found while writing this document.)* |
| 0 | **Restructure to co-located headers and sources** (§8.1) and make `drivers/` a real library target (§8.3). Do it before phase 0 code lands, while the tree is small. |
| 0 | **Move the build to C++20** (§2.2). `set(CMAKE_CXX_STANDARD 20)` in `CMakeLists.txt`, and update `style_guide.md` §1 with the adopted and avoided feature lists. Do it before phase 0 code lands, while only the driver layer exists. |
| 0 | **ISR priorities not assigned.** Anything calling a `FromISR` API must sit at 5–14, set explicitly before `vTaskStartScheduler()`. |
| 2 | **P10** — the VBAT divider ratio is unconfirmed. Measure it before the brownout threshold means anything. |
| 3 | **`Encoder` is written for quadrature** and the hardware is single-channel input capture. A rewrite, not a tweak; phase 3 cannot start without it. |
| 3 | **F7** — the 64-bit encoder accumulator is not atomic on an M33. The single-control-task design dissolves this; record that as the reason in the header. |
| 1 | **`Motor` does not own its ENABLE pin.** The constructor takes PMODE and FAULT but not nSLEEP. On this board ENABLE is per-channel (PA5, PB2) and currently has no owner in the driver layer. `Motor` should take it, so that `onDisable()` can de-assert nSLEEP rather than merely commanding zero duty. *(New — found while writing this document.)* |
| 1 | **`Drivetrain` assumes a single shared ENABLE pin** (`enablePort`/`enablePin`, documented as PA4) and its header comments carry stale pin references — PC1 and PB2 as fault lines, `&htim2` for encoders, PA5/INP19 as an IPROPI. None of that matches the current `.ioc`. See §10.2. *(New — found while writing this document.)* |
